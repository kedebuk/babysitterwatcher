...
export function useCreateEvent() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (event: { daily_log_id: string; time: string; type: string; detail?: string; amount?: number; unit?: string; status?: string; photo_url?: string; photo_url_after?: string; created_by?: string; latitude?: number; longitude?: number }) => {
