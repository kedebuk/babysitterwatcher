...
import { Baby, LogIn, UserPlus, Shield } from 'lucide-react';
    ...
    <div className="flex min-h-screen items-center justify-center p-4">
      <Card className="w-full max-w-sm animate-fade-in border-0 shadow-lg">
        <CardHeader className="text-center pb-2">
          <div className={`mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-2xl ${loginMode === 'admin' ? 'bg-destructive' : 'bg-primary'}`}>
            {loginMode === 'admin' ? <Shield className="h-8 w-8 text-destructive-foreground" /> : <Baby className="h-8 w-8 text-primary-foreground" />}
          </div>
          <h1 className="text-2xl font-bold">{loginMode === 'admin' ? 'Admin Login' : 'Child Tracker'}</h1>
          <p className="text-sm text-muted-foreground">{loginMode === 'admin' ? 'Masuk sebagai administrator' : 'Pantau aktivitas si kecil'}</p>
          <div className="flex gap-2 mt-3">
            ...
            >
              <Baby className="mr-1.5 h-4 w-4" /> Parent / Sitter
            </Button>
            <Button
              variant={loginMode === 'admin' ? 'destructive' : 'outline'}
              size="sm"
              className="flex-1"
          ...
          {loginMode === 'admin' ? (
            <form onSubmit={handleLogin} className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input id="admin-password" type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} required className="h-12 text-base" />
              </div>
              <Button type="submit" variant="destructive" className="w-full h-12 text-base font-semibold" disabled={loading}>
                <Shield className="mr-2 h-5 w-5" /> Masuk sebagai Admin
              </Button>
              <p className="text-xs text-center text-muted-foreground mt-2">
                Belum punya akun admin?{' '}
                <button type="button" className="text-primary hover:underline font-medium" onClick={() => navigate('/admin-setup')}>
                  Daftar Admin
                </button>
              </p>
            </form>
